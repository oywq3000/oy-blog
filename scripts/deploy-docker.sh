...
SERVER_HOST="100.110.148.14"                # 服务器 IP（Tailscale IP）
REMOTE_DIR="/opt/oy-blog"                   # 服务器部署目录
...
ssh "${SSH}" "mkdir -p ${REMOTE_DIR}/build ${REMOTE_DIR}/docker ${REMOTE_DIR}/logs"
for entry in "${SERVICES[@]}"; do
  IFS='|' read -r name mod jar <<< "${entry}"
  ssh "${SSH}" "mkdir -p ${REMOTE_DIR}/build/${name}"
  scp -q "${mod}/target/${jar}" "${SSH}:${REMOTE_DIR}/build/${name}/"
  echo "   OK: ${name}/${jar}"
done
scp -q deploy/docker/Dockerfile          "${SSH}:${REMOTE_DIR}/docker/"
scp -q deploy/docker-compose.yml         "${SSH}:${REMOTE_DIR}/"
scp -q deploy/docker-compose.env.example "${SSH}:${REMOTE_DIR}/deploy.env.example"

# 首次部署：服务器 .env 不存在则生成（本机 .env 的真实密钥 + 模板补全缺失键）
if ! ssh "${SSH}" "test -f ${REMOTE_DIR}/.env"; then
  tmp_env="$(mktemp)"
  ...
  scp -q "${tmp_env}" "${SSH}:${REMOTE_DIR}/.env"
  rm -f "${tmp_env}"
...
# ---- 4. 服务器构建 + 启动 ----
echo "==> [4/5] 服务器构建镜像并启动"
ssh "${SSH}" "cd ${REMOTE_DIR} && docker compose config -q && docker compose up -d --build"

# ---- 5. 状态确认 ----
echo "==> [5/5] 容器状态"
ssh "${SSH}" "cd ${REMOTE_DIR} && docker compose ps"
echo ""
echo "部署完成，验证清单："
